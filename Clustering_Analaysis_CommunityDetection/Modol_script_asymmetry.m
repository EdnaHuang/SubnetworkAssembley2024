function Modol_script_asymmetry(sim_type, path)
%%
addpath ("Cossart Lab");
...
if strcmp (sim_type, 'Cosine') 
    ...
    folder_name= 'Cosine_CommDetectAsymmetry_Modol';
    ...
    nreps = 100;
    %make the folder in the current working directory to save all outpus
    folder_name= 'Cov_CommDetectAsymmetry_Modol';
    output_path = strcat(path, folder_name, "/");
    if ~exist(output_path, 'dir')
       mkdir(output_path)
    end 
...
%Community detection--asymmetric and signed null model
%Ref: compares observed matrix with a "degree-preserving" model. 
% This model handles pos/neg correlations/covariance separately.
%(see Rubinv & Sporns 2011,Weight-conserving ...).
...
<<<<<<< HEAD
 % the repeat # for each gamma value used for community_louvain function, should run several iterations
=======
...
save (fullfile(output_path, 'CommDetectAsymmetry.mat'));
close all
...
saveas(gcf, fullfile(output_path,'Sorted_Rastermap.fig'));
close all
...
saveas(gcf, fullfile(output_path,'cluster_on_detected_events.fig'));
saveas(gcf, fullfile(output_path,'cluster_on_detected_events.jpeg'));
<<<<<<< HEAD
close all;
save (fullfile(output_path,'all.mat')); % Edited AB - 07/25/2022 (Added on request of Edna)
=======
close all
>>>>>>> upstream/main
