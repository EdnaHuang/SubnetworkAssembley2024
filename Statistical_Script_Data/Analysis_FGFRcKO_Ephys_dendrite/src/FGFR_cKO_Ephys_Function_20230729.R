...
lmer_two_way = function(x, data = df_lite) {
  mod = lmer(paste(x, "~ genotype_cre * sex + (1 | subject_id) + (1 | subject_id:section_id)"), data = data, REML = TRUE)
  return(mod)
}
...
model_select = function(x, data = df_lite) {
  ...
  if (isSingular(mod) == TRUE) {
    mod = lmer(paste(x, "~ genotype_cre * sex + (1 |subject_id)"), data = data, REML = TRUE)
  }
  
  if (isSingular(mod) == TRUE) {
    f = as.formula(paste(x, "~ genotype_cre * sex"))
...
plot_raw = function(var, data = df_lite, title = var,
  ...
  df_lite %>%
    select(all_of(var), subject_id, genotype_cre) %>%
    ggplot(aes(x = genotype_cre, y = get(var),
               group = genotype_cre, color = genotype_cre)) +
