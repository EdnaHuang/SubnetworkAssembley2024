...
lmer_three_way = function(x, data = df_lite) {
  mod = lmer(paste(x, "~ f.age * group * f.layer + (1 | subject_id) + (1 | subject_id:location)"), data = data, REML = TRUE)
  return(mod)
}
...
model_select = function(x, data = df_lite) {
  ...
  # main effects only
  mod0 = lmer(paste(x, "~ f.age + group + f.layer + (1 | subject_id) + (1 | subject_id:location)"), data = data, REML = TRUE)
  
  if (isSingular(mod0) == TRUE) {
    mod0 = lmer(paste(x, "~ f.age + group + f.layer + (1 | subject_id)"), 
                data = data, REML = TRUE)
  }
  
  if (isSingular(mod0) == TRUE) {
    f = as.formula(paste(x, "~ group + f.age + f.layer"))
    # mod0 = lm(paste(x, "~ f.age + group + f.layer"), data = data)
    mod0 = eval(bquote(lm(.(f), data = data)))
  }
  
  # add one-way interaction
  mod1 = update(mod0, . ~ . + f.layer:group)
  mod2 = update(mod0, . ~ . + f.age:group)
  mod3 = update(mod0, . ~ . + f.age:f.layer)
  
  # add two way interaction
  mod4 = update(mod1, . ~ . + f.age:group)
  mod5 = update(mod1, . ~ . + f.age:f.layer)
  mod6 = update(mod2, . ~ . + f.age:f.layer)
  
  # add three-way interaction
  mod7 = update(mod4, . ~ . + f.age:f.layer + f.age:group:f.layer)
