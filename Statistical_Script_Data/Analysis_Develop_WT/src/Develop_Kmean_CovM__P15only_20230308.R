...
theme_set(theme_bw())
source(here("src", "Develop_P15only_Function_20230308.R"))
...
# mod_test = model_select(rv_list[5])
mod_list_select = lapply(rv_list, model_select)
names(mod_list_select) = rv_list
tab_model(mod_list_select)
tab_model(mod_list_select[1:5])
tab_model(mod_list_select[6:8])
tab_model(mod_list_select[9:12])
tab_model(mod_list_select[13:15])
...
save.image(file = here("output","2023-03-08_SummaryDevAll_Kmean_CovM_withCorr_Dist_P15only.RData"))
