...
merge_layer_plot_raw_boot = function(x, var_name = x){
  ...
  mod_data$xvar = mod_data$f.age
  mod_data$tvar = mod_data$sex
  emm_plot = emmip(mod_boot, sex ~ f.age, CIs = TRUE, 
                   CIarg = list(lwd = 2, alpha = 0.7), 
                   dodge = 0.5) +
    geom_beeswarm(data = mod_data, dodge.width = 0.5, alpha = 0.5, shape = 1)+
    labs(title = var_name, x = "Age", y = "", color = "Sex")
  
  print(emm_plot)
}


## Merge layer, then report interaction between age and sex---------

merge_layer_report_age_int_sex = function(x, var_name = x) {
  
  # extract replicates
  mod_boot = boot_model_extract(x)
  
  # simple comparisons
  emm = emmeans(mod_boot, ~ f.age * sex)
  ...
  # sex
  emm_contr_sex = contrast(emm, "consec", simple = "sex")
  emm_param_sex = as.data.frame(model_parameters(
             emm_contr_sex, centrality = c("median", "mean"), test = "pd"))
  emm_param_sex$pval = pd_to_p(emm_param_sex$pd)
  emm_param_sex$Median.1 = NULL
  ...
  # print formatted tables in sequence
  table = kable (list(emm_param_age, emm_param_sex), digits = 3)
  print(table)
}


## Merger Sex, then plot raw data and bootstrapping ------------------
merge_sex_plot_raw_boot = function(x, var_name = x){
  ...
  emm_plot = emmip(mod_boot, f.layer ~ f.age, CIs = TRUE, 
                   ...
                   dodge = 0.5) +
    geom_beeswarm(data = mod_data, dodge.width = 0.5, alpha = 0.5, shape = 1)+
    labs(title = var_name, x = "Age", y = "", color = "f.layer")+
...
## merge sex, report age interacting with sex --------------------------------
merge_sex_report_age_int_layer = function(x, var_name = x) {
  
  # extract replicates
  mod_boot = boot_model_extract(x)
  
  # simple comparisons
  emm = emmeans(mod_boot, ~ f.age * f.layer)
  ...
  # print formatted tables in sequence
  table = kable (list(emm_param_age, emm_param_layer), digits = 3)
  print(table)
}


## report sex inter layer when there is no interaction in age-sex/layer---------
  report_sex_int_layer = function(x, var_name = x) {
  
  # extract replicates
  mod_boot = boot_model_extract(x)
  
  # simple comparisons
  emm = emmeans(mod_boot, ~ sex* f.layer)
  
  #compare
  emm_contr_layer = contrast(emm, method = "pairwise")
  emm_param_layer = as.data.frame(model_parameters(
    emm_contr_layer, centrality = c("median", "mean"), test = "pd"))
  emm_param_layer$pval = pd_to_p(emm_param_layer$pd)
  emm_param_layer$Median.1 = NULL
  ...
  # print formatted tables in sequence
  table = kable (emm_param_layer, digits = 3)
  print(table)
}
