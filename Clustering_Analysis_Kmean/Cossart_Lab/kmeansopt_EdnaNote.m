...
%     IDX = kmeans(E',NCl)'; %Normal K-means on distance metric
    ...
    s = silh(M,IDX);
    IDX0(k,:) = IDX; %IDX0 stores all the idx from N loop for a specific cluster number
    S(k) = mean(s);%S stores all the mean of silhValue from N loop for a specific cluster number
